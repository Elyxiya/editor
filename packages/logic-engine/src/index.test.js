...
const makeSetVarNode = (id, varName, value) => ({
    id,
    type: 'setVariable',
    category: 'data',
    label: '设置变量',
...
const makeConditionNode = (id, expression) => ({
    id,
    type: 'condition',
    category: 'logic',
    label: '条件分支',
...
const makeShowMessageNode = (id, content, type = 'info') => ({
    id,
    type: 'showMessage',
    category: 'action',
    label: '显示消息',
...
const makeDelayNode = (id, duration) => ({
    id,
    type: 'delay',
    category: 'action',
    label: '延迟',
...
const makeConnection = (source, target, condition) => ({
    id: `${source}-${target}`,
    source,
    target,
    condition,
});
const makeFlow = (nodes, connections) => ({
    id: 'flow-1',
    name: '测试流程',
    nodes,
    connections,
        ...
        describe('loop node', () => {
            it('should iterate over items', async () => {
                ...
                const flow = makeFlow([trigger, setItems, loopNode, setItem], [
                    ...
                    makeConnection('loop1', 'setItem', undefined),
                ]);
                const context = await executor.execute(flow);
