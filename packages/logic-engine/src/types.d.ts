...
/** 节点子类型 */
export type NodeSubtype = 'onClick' | 'onChange' | 'onSubmit' | 'onLoad' | 'onMounted' | 'onUnmounted' | 'onTimer' | 'onWebSocket' | 'setValue' | 'callApi' | 'showMessage' | 'showModal' | 'hideModal' | 'navigate' | 'download' | 'upload' | 'condition' | 'switch' | 'loop' | 'delay' | 'parallel' | 'sequence' | 'getVariable' | 'setVariable' | 'transform' | 'filter' | 'sort' | 'aggregate';
/** 节点配置 */
...
/** 逻辑引擎事件类型 */
export type LogicEngineEvent = 'flow:start' | 'flow:end' | 'flow:error' | 'node:enter' | 'node:exit' | 'node:error' | 'condition:evaluate' | 'variable:change';
/** 事件监听器 */
