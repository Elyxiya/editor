...
// 端口定义辅助函数
// ============================================================
const createInput = (id, name, label, type = 'any', required = false, defaultValue) => ({
    id,
    name,
    label,
    type,
...
const createOutput = (id, name, label, type = 'any') => ({
    id,
    name,
    label,
    type,
...
export function getNodesByCategory(category) {
    const filterFn = (nodes) => Object.values(nodes);
    switch (category) {
        case 'trigger':
 ...
 * 获取可拖拽的节点列表
 */
export function getDraggableNodes() {
