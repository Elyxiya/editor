 ...
 * 从节点定义创建节点
 */
export function createNodeFromDefinition(definition, position = { x: 0, y: 0 }) {
 ...
 * 创建触发器节点
 */
export function createTriggerNode(subtype, position = { x: 100, y: 100 }) {
    return createNodeFromDefinition(nodeRegistry.get(subtype), position);
}
/**
 * 创建动作节点
 */
export function createActionNode(subtype, params = {}, position = { x: 100, y: 100 }) {
    return createNodeBuilder()
        .setType(subtype)
        .setPosition(position.x, position.y)
        .setConfig(params)
 ...
 * 创建逻辑节点
 */
export function createLogicNode(subtype, params = {}, position = { x: 100, y: 100 }) {
    return createNodeBuilder()
        .setType(subtype)
        .setPosition(position.x, position.y)
        .setConfig(params)
 ...
 * 创建数据节点
 */
export function createDataNode(subtype, params = {}, position = { x: 100, y: 100 }) {
    return createNodeBuilder()
        .setType(subtype)
        .setPosition(position.x, position.y)
        .setConfig(params)
 ...
 * 创建流程构建器
 */
export function createFlowBuilder(initial) {
    const state = {
 ...
 * 创建简单的点击事件流程
 */
export function createClickEventFlow(componentId, actions) {
    ...
    actions.forEach((action, index) => {
        const actionNode = createActionNode(action.type, action.params, { x: 100, y: yOffset + index * 150 });
 ...
 * 创建表单提交流程
 */
export function createFormSubmitFlow(formId, config) {
 ...
 * 创建数据列表加载流程
 */
export function createListLoadFlow(listId, config) {
 ...
 * 复制流程
 */
export function cloneFlow(flow, newName) {
    // 创建 ID 映射
    const idMap = new Map();
    // 生成新 ID
 ...
 * 计算节点的边界框
 */
export function calculateFlowBounds(flow) {
 ...
 * 自动布局节点
 */
export function autoLayoutFlow(flow, direction = 'vertical') {
